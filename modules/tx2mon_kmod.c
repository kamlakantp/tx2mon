...
static u32 smcc_update_core_mask(uint32_t mask)
{
...
static u32 smcc_update_core_feature_per_cpu(void)
{
	struct arm_smccc_res res;
...
static int update_core_feature(void)
{
	int cpu;
	int err;
			...
			char *buf)
{
	...
	mask = smcc_update_core_mask(0xffff);
	if (mask < 0)
		return -EIO;
			...
			const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 0, &mask) < 0)
		return -EINVAL;
	if (smcc_update_core_mask(mask) < 0)
		return -EIO;
	if (update_core_feature())
		return -EIO;
...
static int __init socmon_init(void)
{
	...
	/* Check if core mask feature supported in firmware */
	err = smcc_update_core_mask(0xffff);
	if (!(err < 0)) {
		err = sysfs_create_file(&tx2mon_data->pdev->dev.kobj,
